...
from agents.delivery_order_agent.graph import run_delivery_order_graph
...
async def stream_handler(request: TaskRequest) -> AsyncIterator[TaskEvent]:
    ...
    yield TaskEvent(
        ...
        message="DeliveryOrderAgent: processing delivery order request",
    )
    await asyncio.sleep(0.05)

    artifact = run_delivery_order_graph(request.message)
    ...
    yield TaskEvent(
        ...
        message="DeliveryOrderAgent: completed",
        artifact=artifact,
    )


def create_app() -> FastAPI:
    _app = FastAPI(title="DeliveryOrderAgent", version="0.1.0")
