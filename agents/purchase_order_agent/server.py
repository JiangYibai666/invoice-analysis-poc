...
from agents.purchase_order_agent.graph import run_purchase_order_graph
...
async def stream_handler(request: TaskRequest) -> AsyncIterator[TaskEvent]:
    ...
    artifact = run_purchase_order_graph(request.message)
    ...
    yield TaskEvent(
        ...
        message="PurchaseOrderAgent: completed",
        artifact=artifact,
    )


def create_app() -> FastAPI:
    _app = FastAPI(title="PurchaseOrderAgent", version="0.1.0")
