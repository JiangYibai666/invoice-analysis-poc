...
Project Title: AI-powered conversational bot based on RAG
...
This industrial attachment project delivered an AI-powered conversational bot based on retrieval-augmented generation (RAG) for Doxa's procurement, finance, and logistics workflows. The delivered system is the final project deliverable and runs in a local environment. It is one functional module within the broader DOXA CONNEX AI agent programme. It focuses on the invoice matching capability under the finance and payment domain, allowing users to query invoice, purchase order (PO), and delivery order (DO) data through natural language and receive explainable business analysis.
...
The delivered system runs in a local environment and is the final project deliverable rather than a throwaway prototype. It demonstrates multi-agent orchestration, natural-language data access, deterministic document matching, conversation memory, long-term memory, task traceability, and a lightweight user interface. These capabilities provide a foundation for a future production-ready invoice matching agent.
...
This project is a functional implementation of that strategy, delivered as a locally runnable application. It connects to existing PostgreSQL data sources, reads invoice, PO, and DO records, exposes multiple agent services through a lightweight A2A HTTP protocol, and supports questions such as:
...
| Overall programme item                      | Project contribution                                                                              |
| ------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| Finance and payment: Invoice Matching Agent | Implements the first working natural-language invoice analysis and document matching deliverable. |
...
The key business tables used by this system include:
...
The business goals were translated into the following functional requirements:
...
Users can run the local application with:
...
The browser frontend also maintains a local conversation scope and shows saved conversations in the left History panel. Users can reopen historical conversations and delete saved conversation records. This makes the application closer to a practical business assistant rather than a one-off query tool.
...
| Week 12 | 2026-05-18 to 2026-05-22 | Started the final deliverable (the RAG-based conversational bot), analysed PostgreSQL database structure, and implemented task/session storage and the Host/Invoice A2A loop. | Invoice database analysis, PostgreSQL task/session store, three invoice query scenarios, interactive CLI.                                        |
...
The overall plan can be summarised into four phases: Week 1-6 covered AI capability validation and business modelling; Week 7-11 covered the A2A and multi-agent architecture POC exploration; Week 12-16 delivered the core final deliverable — this AI-powered conversational bot based on RAG; and Week 17-18 completed the frontend, conversation history, and memory capabilities.
...
| Analysis Workflow            | Week 4-12  | Analysed business roles, core scenarios, database structures, A2A interaction flows, and PostgreSQL data sources required by the final deliverable (the RAG-based conversational bot).   | Business object analysis, database structure analysis, invoice query scenarios, Host/Invoice A2A loop.               |
...
| Implementation Workflow      | Week 8-18  | Implemented the A2A prototype, the final deliverable (RAG-based conversational bot), PO/DO agents, deterministic matching, browser frontend, conversation history, and long-term memory. | Runnable local application, CLI, browser frontend, PO/DO matching, and memory capability.                            |
...
1. **Integrate with Connex APIs and authentication.** The local application should evolve into a service that uses Connex identity, authorisation, tenant boundaries, audit logs, and production API contracts.
...
5. **Upgrade the current memory into governed enterprise memory.** Long-term memory needs privacy controls, retention rules, tenant isolation, deletion workflows, and monitoring for incorrect memory reuse.
...
From a data safety perspective, the project reinforced the need for layered protection. The system uses read-only database connections, SQL validation, dangerous keyword blocking, and result row limits. These are still baseline controls, but they show the correct direction for production hardening.
...
The third improvement would be to separate local-environment assumptions from production requirements more clearly. The current system is suitable for local operation, but production deployment requires tenant isolation, role-based permissions, formal audit logging, data retention policies, monitoring, and stronger SQL safety.
...
This project supports the DOXA CONNEX AI agent roadmap by implementing the first runnable version of the invoice analysis and matching assistant. The overall roadmap targets 16 agents across four months. This final deliverable focuses on the finance and payment domain while also creating reusable infrastructure for later agents.
...
| NFR-07 | Scalability                | Current version is a local application; production should use managed deployment and queues.           |
| NFR-08 | Security and compliance    | Baseline guardrails exist; production requires Connex auth, tenant isolation, and stricter SQL policy. |

**UI Design**

The application UI is intentionally simple for demonstration and validation:
...
The application does not modify source invoice or purchase data. The system creates the following tables in the configured task database:
...
The local application is run with:
...
| Week 12 | 2026-05-18 to 2026-05-22 | Started the final deliverable (RAG-based conversational bot), analysed PostgreSQL structure, and implemented task/session store and Host/Invoice A2A loop. | Database analysis, task/session storage, three invoice queries, CLI.                        |
...
This project can be viewed as the foundation for Invoice Matching Agent v1 in the broader roadmap:

| Month   | Overall programme focus                                                             | Relationship to this project                                                                                                   |
| ------- | ----------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| Month 1 | Discovery: stakeholder interviews, workflow mapping, and use-case prioritisation.   | This project uses invoice matching priority and business workflow assumptions as input.                                        |
| Month 2 | Design: BRD, UX design, agent architecture, API specification, and security review. | This project implements the initial architecture and frontend interaction but does not replace formal BRD and security review. |
| Month 3 | Development: high and medium priority agents, including Invoice Matching Agent v1.  | This project implements the first local version of invoice analysis and matching capability.                                   |
| Month 4 | UAT, performance testing, training, and phased production rollout.                  | This project provides a basis for UAT planning but still requires production hardening.                                        |
